...
| RF-08 | `src/retriever.py`, `src/retrieve.py` | `tests/test_retriever.py` · **evidencia exp.** `93_VALIDACION_RETRIEVER.md` · **evaluación** `94_EVALUACION_RECUPERACION.md` · **análisis de fallos** `95_ANALISIS_FALLOS_RETRIEVER.md` | `diagrams/secuencia_consulta.mmd`, `03_flujos.md` §3.1 |
| … | … | … | … |
...
| ADR-014 | OE3 | RF-07, RF-08 | Retriever (`src/retriever.py`, `src/retrieve.py`) | P-22, P-27, P-31, P-32, P-33 | ✅ |
...
| 2026-06-02 | **Evaluación formal del Retriever (RF-08)**: 12 consultas operativas sobre `tesisaiops_validacion`, relevancia juzgada vs. histograma de códigos reales; Precisión@1=0.42 ponderada (0.67 con parciales), Precisión@3=0.40, Éxito@3=0.67. Amenazas a la validez/limitaciones/trabajo futuro en `docs/94_EVALUACION_RECUPERACION.md`; P-32. **Sin LLM, sin código** | Carlos Valdez |
| 2026-06-02 | **Análisis de fallos del Retriever (RF-08)**: causa raíz por consulta con datos del corpus (timeout=0, auth=13, CORS/API existían y no se recuperaron); fallos duros 75 % ausencia / 25 % modelo (ampliado ≈50/50). Prioriza híbrido > re-ranking; corpus para fenómenos ausentes. `docs/95_ANALISIS_FALLOS_RETRIEVER.md`; P-33. **Sin LLM, sin código** | Carlos Valdez |
